<div id="stageTable">
                    ...
                    <td>                        
                        ...
                        @if($d->stage == 1)
                            <button type="button" class="btn btn-sm btn-deep-gold stage-action-btn"
                                    data-bs-toggle="modal" data-bs-target="#userModal_stage1_{{ $d->id }}">
                                View Info
                            </button>
                        @elseif($d->stage == 2)
                            <button type="button" class="btn btn-sm btn-deep-gold stage-action-btn"
                                    data-bs-toggle="modal" data-bs-target="#userModal_stage2_{{ $d->id }}">
                                View Video
                            </button>
                        @endif
    ...
    {{-- Modals for each user --}}
        @foreach($stage1 as $d)
            <!-- @if($d->stage == 1) -->
                <!-- Modal for Stage 1 -->
                <div class="modal fade" id="userModal_stage1_{{ $d->id }}" tabindex="-1" role="dialog" aria-labelledby="userModalLabel{{ $d->id }}" aria-hidden="true">
        ...
        <!-- Modal for Stage 2 (Video) -->
                <div class="modal fade" id="userModal_stage2_{{ $d->id }}" tabindex="-1" role="dialog" aria-labelledby="userModalVideoLabel{{ $d->id }}" aria-hidden="true">
                <div class="modal-dialog modal-lg" role="document">
                    <div class="modal-content">
                    <form id="reviewForm_{{ $d->id }}">
                        ...
                        <div class="modal-header bg-primary text-white">
                        <h5 class="modal-title" id="userModalVideoLabel{{ $d->id }}">Craft Video Review</h5>
                        ...
                        <video width="100%" controls class="mb-3">
                            <source src="{{ url('public/video/phase1/' . $d->content) }}" type="video/mp4">
                        </video>
                        <div class="form-group">
                            <label for="comment_{{ $d->id }}">Comment</label>
                            <textarea id="comment_{{ $d->id }}" name="comment" class="form-control" rows="3"></textarea>
                        </div>
                        <div class="form-group">
                            <label for="status_{{ $d->id }}">Review Status</label>
                            <select id="status_{{ $d->id }}" name="status" class="form-control" required>
