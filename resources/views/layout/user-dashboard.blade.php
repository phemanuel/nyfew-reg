...
function fetchStageData(stage, page = 1) {
    $.ajax({
        url: "/fetch-stage-data/" + stage + "?page=" + page,
        type: 'GET',
        beforeSend: function () {
            $('#stageTable').html('<div class="text-center my-3">Loading...</div>');
        },
        success: function (response) {
            // Replace HTML with new stage data
            $('#stageTable').html(response.html);

            // Bootstrap 5 automatically initializes modals from HTML
            // No need for manual .modal() call like in Bootstrap 4
        },
        error: function () {
